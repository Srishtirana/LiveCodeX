...
if (cluster.isPrimary) {
  ...
  async function processSubmission(submission: any) {
    ...
    const dockerMountPath = absoluteCodeDir.replace(/\\/g, "/");
    ...
    try {
      ...
      switch (language) {
        case "javascript":
          ...
          dockerCommand = `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${dockerMountPath}:/usr/src/app" node:18 sh -c "node /usr/src/app/${path.basename(
            codeFilePath
        ...
        case "python":
          ...
          await fs.writeFile(codeFilePath, code);
          dockerCommand = `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${dockerMountPath}:/usr/src/app" python:3.9 sh -c "python /usr/src/app/${path.basename(
            codeFilePath
          )} < /usr/src/app/input.txt"`;
          break;
...
-v "${dockerMountPath}:/usr/src/app" gcc:11  \
sh -c "g++ /usr/src/app/userCode.cpp -o /usr/src/app/a.out && /usr/src/app/a.out < /usr/src/app/input.txt"`;
        ...
        case "rust":
          ...
          await fs.writeFile(codeFilePath, code);
          dockerCommand = `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${dockerMountPath}:/usr/src/app" rust:latest sh -c "rustc /usr/src/app/userCode.rs -o /usr/src/app/a.out && /usr/src/app/a.out < /usr/src/app/input.txt"`;
          break;

        case "java":
          codeFilePath = path.join(absoluteCodeDir, "UserCode.java");
          await fs.writeFile(codeFilePath, code);
          dockerCommand = `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${dockerMountPath}:/usr/src/app" openjdk:17 sh -c "javac /usr/src/app/UserCode.java && java -cp /usr/src/app UserCode < /usr/src/app/input.txt"`;
          break;

        case "go":
          codeFilePath = path.join(absoluteCodeDir, "userCode.go");
          await fs.writeFile(codeFilePath, code);
          dockerCommand = `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${dockerMountPath}:/usr/src/app" golang:1.18 sh -c "go run /usr/src/app/userCode.go < /usr/src/app/input.txt"`;
          break;
    ...
    exec(dockerCommand, { timeout: 10000 }, async (error, stdout, stderr) => {
      let result = [stdout, stderr].filter(Boolean).join("\n").trim();
      if (error) {
        result = result || `Error: ${error.message}`;
      }
      if (!result) result = "Program finished successfully with no output.";
