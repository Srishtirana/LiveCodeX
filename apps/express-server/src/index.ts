...
function isCommandMissing(result: RunResult): boolean {
  return (
...
async function runLocally(
  ...
  try {
    ...
    const dockerCommands: Record<
      string,
      { file: string; command: string }
    > = {
      cpp: {
        file: "userCode.cpp",
        command: `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${mountPath}:/usr/src/app" gcc:11 sh -c "g++ /usr/src/app/userCode.cpp -o /usr/src/app/a.out && /usr/src/app/a.out < /usr/src/app/input.txt"`,
      },

      rust: {
        file: "userCode.rs",
        command: `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${mountPath}:/usr/src/app" rust:latest sh -c "rustc /usr/src/app/userCode.rs -o /usr/src/app/a.out && /usr/src/app/a.out < /usr/src/app/input.txt"`,
      },

      go: {
        file: "userCode.go",
        command: `docker run --rm --memory="256m" --cpus="1.0" --pids-limit 100 -v "${mountPath}:/usr/src/app" golang:1.20 sh -c "go run /usr/src/app/userCode.go < /usr/src/app/input.txt"`,
      },
    };
    ...
    await fs.writeFile(
      path.join(runDir, dockerRun.file),
      code,
      "utf8"
    );
    ...
    if (
      ...
      return {
        output:
          "Docker is required to run C++, Rust, and Go on this server.",
        success: false,
      };
    }
...
app.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    console.log(
      `Received submission from room ${roomId} | language: ${language}`
    );
...
app.get(
  "/",
  (_req: Request, res: Response): void => {
    res.send("LiveCodeX backend running");
  }
);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("Redis unavailable — running locally");
  }
}
